...
export class RdsService extends Construct {
  readonly clusterEndpointHostname: string;
  readonly clusterIdentifier: string;
  readonly secret: secretsmanager.ISecret;
  readonly connections: ec2.Connections;

  constructor(scope: Construct, id: string, props: RdsServiceProps) {
    super(scope, id);
  ...
  private _createRdsInstance(props: RdsServiceProps): DatabaseConfig {
  ...
  private _createRdsCluster(props: RdsServiceProps): DatabaseConfig {
    const vpc = props.vpc ? props.vpc : getOrCreateVpc(this);
